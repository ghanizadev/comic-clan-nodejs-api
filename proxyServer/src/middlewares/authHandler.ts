...
export default (scopes : string[]) => {
    return (req : Request, res : Response, next : NextFunction) => {
        try {
            ...
            eventHandler.publish('users_ch', {event: 'single', body: {query: {email: payload.username} }})
            .then((user) => {

                if(!user)
                    throw new HTTPError('unauthorized_client', 'User was not found or it was deleted', 403);

                const length = user.payload.scopes.filter((s : string) => scopes.includes(s)).length;

                if( length === 0 || length !== scopes.length)
                    throw new HTTPError('unauthorized_client', `User must have ${scopes.toString()} scopes in order to access this endpoint`, 403);
